...
export function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const storedPassword =
      localStorage.getItem("@AgileTask:password") || "123456";

    if (twoFactorEnabled && !needsTwoFactor) {
      if (email === storedEmail && password === storedPassword) {
        setNeedsTwoFactor(true);
        toast.info(
          "Digite o código de autenticação em dois fatores para continuar.",
        );
        return;
      }
    ...
    if (twoFactorEnabled && needsTwoFactor) {
      const savedCode = localStorage.getItem("@AgileTask:twoFactorCode") || "";
      if (twoFactorCode.trim() !== savedCode) {
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-2 text-center max-w-[250px]">
            Bem-vindo de volta! Faça login para gerenciar suas tarefas.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          {needsTwoFactor && (
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1.5">
                No protótipo, o código aparece em Configurações → Perfil.
              </p>
            </div>
          )}
          ...
          <button
            type="submit"
            className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg shadow-blue-500/30 transition-all active:scale-[0.98] mt-2"
          >
            {needsTwoFactor ? "Validar código" : "Entrar"}
          </button>
        </form>
        ...
        <button
          ...
          className="mt-6 w-full py-3 px-4 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold rounded-xl flex items-center justify-center gap-3 transition-colors active:scale-[0.98]"
        >
